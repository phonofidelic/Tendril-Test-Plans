#!/usr/bin/env bash
# Provision trycua computer-server inside a macOS Lume VM.
# Run THIS SCRIPT INSIDE THE VM (e.g. ssh user@vm 'bash -s' < provision-computer-server.sh).
...
# See the parent SKILL.md, section 4.
set -euo pipefail

PORT="${CUA_SERVER_PORT:-8443}"
